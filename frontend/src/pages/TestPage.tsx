...
export default function TestPage() {
  ...
  const urlParamModel = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("model") : null
  const [model, setModel] = useState(urlParamModel || "combo-auto")
    ...
    (async () => {
      try {
        ...
        if (options.length) {
          ...
          setModel(current => chooseDefaultModel(options, current, urlParamModel || undefined))
        }
      } catch {
        // keep fallback list
      }
