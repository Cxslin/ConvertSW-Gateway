...
export function getStoredApiKey(): string {
  try {
    const stored = localStorage.getItem('convertsw_api_key') || localStorage.getItem('qwen2api_key')
...
export function setStoredApiKey(input: string): string {
  ...
  if (!key) return ""
  localStorage.setItem('convertsw_api_key', key)
  localStorage.setItem('qwen2api_key', key)
  return key
}

export function clearStoredApiKey() {
  localStorage.removeItem('convertsw_api_key')
