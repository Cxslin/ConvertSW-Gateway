...
const navItems = [
  ...
  { name: "Model Combo", path: "/combos", icon: Layers },
  { name: "Provider AI", path: "/providers", icon: Cpu },
      ...
      >
        <div className="flex h-20 items-center justify-between border-b border-border/45 px-6">
          <Link to="/" onClick={() => setMobileOpen(false)} className="group min-w-0">
            <div className="text-[10px] font-black uppercase tracking-[0.32em] text-muted-foreground">Router AI Wrap</div>
            <div className="mt-0.5 truncate text-xl font-black tracking-tight transition group-hover:text-primary">ConvertSW-Gateway</div>
          </Link>
          <button
            type="button"
          ...
          {navItems.map(item => {
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={() => setMobileOpen(false)}
                className={`group flex items-center gap-3 whitespace-nowrap rounded-2xl border px-3.5 py-2.5 text-sm font-bold transition ${
                  active
        ...
        <div className="border-t border-border/45 p-4">
          <div className="rounded-[24px] border border-white/70 bg-muted/22 p-4 shadow-sm">
            <div className="flex items-center gap-2 text-xs font-black text-muted-foreground">
              ...
              Gateway Router Aktif
            </div>
            <div className="mt-1.5 text-xs leading-5 text-muted-foreground">
              DeepSeek · ChatGPT · Mistral · Qwen
              <br />
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/70 bg-card/76 px-4 shadow-sm backdrop-blur-xl md:h-20 md:px-8">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="text-[11px] font-black uppercase tracking-[0.24em] text-muted-foreground">Gateway Console</div>
