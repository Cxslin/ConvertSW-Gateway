...
func (app *App) uploadLocalFileToUpstream(ctx context.Context, acc *Account, local UploadedLocalFileRecord) (map[string]any, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, helperDaemonURL("/upload"), bytes.NewReader(reqBody))
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to call upload daemon: %w", err)
	}
	...
	if err := json.Unmarshal(respBytes, &res); err != nil {
		return nil, fmt.Errorf("invalid upload daemon response: %s", string(respBytes))
	}
	if !res.OK || res.File.ID == "" {
		return nil, fmt.Errorf("upload to DeepSeek failed: %s", res.Error)
	}
